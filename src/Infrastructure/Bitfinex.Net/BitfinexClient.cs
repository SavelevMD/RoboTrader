...
namespace Bitfinex.Net
{
    ...
    public class BitfinexClient : RestClient, IBitfinexClient
    {
        ...
        private const string SubmitOrderEndpoint = "auth/w/order/submit";
        ...
        /// <summary>
        /// SubmitOrder (try open and process order)
        /// </summary>
        /// <param name="type">type of order</param>
        /// <param name="symbol">pair of currencies</param>
        /// <param name="price">price</param>
        /// <param name="amount">amount</param>
        /// <param name="cancellationToken">token</param>
        /// <returns>http response</returns>
        public async Task<WebCallResult<TResult>> SubmitOrderAsync<TResult>(string type, string symbol, string price, string amount, CancellationToken cancellationToken = default) 
            where TResult : class
        {
            var parameters = new Dictionary<string, object>();
            ...
            return await SendRequest<TResult>(GetUrl(SubmitOrderEndpoint, ApiVersion2), HttpMethod.Post, cancellationToken, parameters, true);
        }
        ...
        protected override Error ParseErrorResponse(JToken data)
        {
            if (!(data is JArray))
            {
                ...
                {
                    return new ServerError(data["code"]?.ToObject<int>() ?? -1, data["error"] + ": " + data["error_description"]);
                }                
                if (data["message"] != null)
                    return new ServerError(-1, data["message"]?.ToString() ?? string.Empty);
                else
                    return new ServerError(-1, data.ToString());
            }

            var error = data.ToObject<BitfinexError>();
            return new ServerError(error?.ErrorCode ?? -1, error?.ErrorMessage ?? string.Empty);
